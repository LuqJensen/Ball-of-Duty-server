...
namespace EntityImpl.Components.Physics.Collision
{
    [Serializable]
    public class CollisionHandler : ICollisionHandler
    {
        public bool IsColliding(IBody b1, IBody b2)
        {
            bool retval = false;
            if (b1.Type == Geometry.CIRCLE && b2.Type == Geometry.CIRCLE)
            {
                retval = CollisionCircleCircle(b1, b2);
            }
            else if (b1.Type == Geometry.CIRCLE && b2.Type == Geometry.RECTANGLE)
            {
                retval = CollisionCircleRectangle(b1, b2);
            }
            else if (b1.Type == Geometry.RECTANGLE && b2.Type == Geometry.CIRCLE)
            {
                retval = CollisionCircleRectangle(b2, b1);
            }
            else if (b1.Type == Geometry.RECTANGLE && b2.Type == Geometry.RECTANGLE)
            {
        ...
        public bool CollisionLineSquare(double x1, double x2, double y1, double y2, IBody b2)
        {
        ...
        private bool CollisionCircleCircle(IBody b1, IBody b2)
        {
        ...
        public int GetFirstObjectIntersectingPath<T>(ICollection<IGameObject> gameobjects, IGameObject gameobjectWithPath, int mapSize)
        {
            double closestDistance = -1;
            int gameobjectTId = 0;
            Point center1 = gameobjectWithPath.Body.Center;
            double cx = center1.X;
            double cy = center1.Y;
            double diameter = gameobjectWithPath.Body.Height;
            double radius = diameter / 2;

            Vector velocity = gameobjectWithPath.Physics.Velocity;
            ...
            Vector radiusVec = velocity;
            radiusVec.Normalize();

            radiusVec = Vector.Multiply(radiusVec, radius);

            double deltaX = velocity.X * mapSize;
            double deltaY = velocity.Y * mapSize;
            ...
            return gameobjectTId;
        }

        public bool CollisionCircleRectangle(IBody circle, IBody rect)
        {
        ...
        public bool CollisionRectangleRectangle(IBody rect1, IBody rect2)
        {
